...
conn.execute(
    """
CREATE TABLE IF NOT EXISTS weekly_intel_100 (
    name TEXT,
    volume_0 INTEGER,
    volume_1 INTEGER,
...
conn.execute(
    "DELETE FROM weekly_intel_100 WHERE date_generated = ?",
    (intel_df["date_generated"][0],),
)
intel_df.to_sql("weekly_intel_100", conn, if_exists="append", index=False)
conn.close()
