...
def clean_price(text):
    # Remove currency symbol, commas, spaces, and emojis/symbols
    text = re.sub(r"[^\d.\-]", "", text)

    # Handle 'K' or 'M' manually if needed
    if "K" in text.upper():
        return round(float(re.sub(r"[^\d.]", "", text)) * 1_000, 2)
    elif "M" in text.upper():
        return round(float(re.sub(r"[^\d.]", "", text)) * 1_000_000, 2)
    else:
