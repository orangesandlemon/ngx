...
for i in tqdm(range(start_index, len(all_days))):
 ...
 # Aggregations
    ...
    avg_vol_0 = p0.groupby("name")["volume"].mean().reset_index().rename(columns={"volume": "avg_vol_0"})
    avg_vol_1 = p1.groupby("name")["volume"].mean().reset_index().rename(columns={"volume": "avg_vol_1"})
...
# Merge all
    ...
    p1_today["volume_spike_today"] = p1_today["volume"] > 1.3 * p1_today["volume_avg_10"]
...
intel_df["sustained_volume_buildup"] = intel_df["avg_vol_1"] > 1.3 * intel_df["avg_vol_0"]
...
intel_df['stealth_accum_candidate'] = (
    intel_df['volume_slope'] & (intel_df['avg_change_1'].abs() < 1.5)
)
intel_df['momentum_spike'] = (
    (intel_df['avg_change_1'] > 2.0) & intel_df['volume_spike']
)
intel_df['combo_stealth_buildup'] = intel_df['stealth_accum_candidate'] & intel_df["sustained_volume_buildup"]
...
intel_df['combo_early_momentum'] = intel_df['volume_spike'] & (~intel_df["breakout_high"])
...
intel_df['score'] = (
        intel_df[
        [
            'volume_spike', 'stealth_accum_candidate',
            "price_flip_up", 'momentum_spike', "volume_slope", "breakout_high","flat_high_volume",
            'combo_flat', 'combo_reversal', 'combo_climb', 'combo_early_momentum','momentum_spike1','sustained_volume_buildup'
        ]
    ].sum(axis=1)
...
intel_df["trend_reversal_flag"] = (intel_df["avg_change_0"] > 0) & (intel_df["avg_change_1"] < 0)
intel_df["volume_drop_flag"] = intel_df["volume_1"] < intel_df["volume_0"] * 0.85  # Optional
...
intel_df.drop(columns=["price_drop_flag", "trend_reversal_flag", "volume_drop_flag", "sustained_volume_buildup"], inplace=True)



# Save to DB
conn.execute("""
CREATE TABLE IF NOT EXISTS weekly_intel_short (
    name TEXT,
    volume_0 INTEGER,
    volume_1 INTEGER,
...
conn.execute("DELETE FROM weekly_intel_short WHERE date_generated = ?", (intel_df['date_generated'][0],))
intel_df.to_sql("weekly_intel_short", conn, if_exists='append', index=False)
conn.close()
