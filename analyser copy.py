...
# === TOPS
top_gainers = today_df.nlargest(3, "change")[["name", "change"]]
top_losers = today_df.nsmallest(3, "change")[["name", "change"]]
top_volume = today_df.nlargest(3, "volume")[["name", "volume"]]
top_volume["volume"] = top_volume["volume"].apply(lambda x: f"{int(x):,}")
...
def format_reason_text(reasons, row):
    ...
    if "Low Trade Count" in reasons:
        expl.append("Few trades despite volume — signs of big buyers entering quietly.")

    if "High Trade Count" in reasons:
...
📈 Top Gainers (Based on True Close Change %):
...
🧠 Signals Generated: {len(signals_df)} stock(s) triggered accumulation or setup alerts.
"""
...
- No major signals triggered today.
- Watch MAYBAKER: if price moves above ₦6.95, breakout likely.
- Avoid chasing UCAP — high trades, low value = retail-driven.
"""
...
def send_email(subject, body):
    ...
    msg["From"] = formataddr(("NGX Signal Bot", EMAIL_ADDRESS))
    msg["To"] = TO_EMAIL
    ...
    for _, row in signals_df.iterrows():
        ...
        if row["signal"] == "Institutional Accumulation":
            tomorrow_actions.append(
                f"- {row['name']}: Showing signs of smart buying. Consider entry if it stays above ₦{row['close']:.2f}."
            )
        elif row["signal"] == "Retail Buying Frenzy":
            tomorrow_actions.append(
                f"- {row['name']}: Spiking on retail activity. Wait for dip or avoid."
            )
        elif row["signal"] == "Distribution Exit":
            tomorrow_actions.append(
                f"- {row['name']}: Weakness showing. Consider exiting if it drops below ₦{row['close'] - 0.5:.2f}."
            )
        elif row["signal"] in ["🚀 Limit-Up Breakout", "💥 Institution Moon Alert"]:
            tomorrow_actions.append(
                f"- {row['name']}: Momentum strong. Set alert at ₦{row['close'] + 0.5:.2f} and watch volume tomorrow."
            )
        elif row["signal"] == "🚨 Limit-Up Watch":
            tomorrow_actions.append(
                f"- {row['name']}: First limit-up. Watch closely — could rally further."
            )
        elif row["signal"] == "⚠️ Setup Detected":
            tomorrow_actions.append(
...
summary_text = f"""📊 NGX Summary for {latest_day.strftime('%Y-%m-%d')}
...
# === Email Body
signal_table_header = f"{'Stock':<12} {'Signal':<22} {'Change':<10} {'Confidence':<10} {'Action':<12} Buy Range"
...
<h2>📊 NGX Summary for {latest_day.strftime('%Y-%m-%d')}</h2>

<div class="section">
  <strong>Top Gainers</strong>
  <pre>{top_gainers.to_string(index=False)}</pre>

  <strong>Top Losers</strong>
  <pre>{top_losers.to_string(index=False)}</pre>

  <strong>Top Volume</strong>
  <pre>{top_volume.to_string(index=False)}</pre>
</div>

<div class="section">
  <strong>🧭 What You Should Do Tomorrow</strong>
  <ul>
    {''.join(f"<li>{line}</li>" for line in tomorrow_actions) if tomorrow_actions else "<li>No actions needed. Watch list only.</li>"}
  </ul>
</div>

<div class="section">
  <strong>📍 Signal Highlights</strong>
  <table>
    <tr>
      <th>Stock</th><th>Signal</th><th>Change</th><th>Confidence</th><th>Action</th><th>Buy Range</th>
    </tr>
    {signal_rows}
  </table>
</div>
...
<div class="section">
  <em>🕒 Summary generated at {datetime.now().strftime('%I:%M %p %Z')}</em>
</div>

</body>
</html>
"""
# === Append trend summary block AFTER full_email is defined
...
if not trend_only.empty:
    ...
    full_email += "<div class='section'><strong>📈 Trend-Based Setups:</strong><ul>"
    for _, row in trend_only.iterrows():
        full_email += f"<li>{row['name']} — {row['signal']} (Score: {row['confidence_score']}/100)</li>"
    full_email += "</ul></div>"

    full_email += "</body></html>"  # Add it back
